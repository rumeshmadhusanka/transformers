...
@add_start_docstrings("""T5 Model with a two decoders.""", T5_START_DOCSTRING)
class T5WithMultiDecoders(T5PreTrainedModel):
    _keys_to_ignore_on_load_missing = [
        r"encoder\.embed_tokens\.weight",
        r"decoder\.embed_tokens\.weight",
        r"lm_head\.weight",
    ...
    def __init__(self, config):
        ...
        decoder_config.is_encoder_decoder = False
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder0 = T5Stack(decoder_config, self.shared)
        self.decoder1 = T5Stack(decoder_config, self.shared)
        self.current_decoder = self.decoder0
        self.current_decoder_idx = 0
    ...
    def parallelize(self, device_map=None):
        ...
        assert_device_map(self.device_map, len(self.encoder.block))
        self.encoder.parallelize(self.device_map)
        self.decoder0.parallelize(self.device_map)
        self.decoder1.parallelize(self.device_map)
        self.lm_head = self.lm_head.to(self.decoder.first_device)
        self.model_parallel = True
    ...
    def deparallelize(self):
        ...
        self.decoder.deparallelize()
        self.encoder = self.encoder.to("cpu")
        self.decoder0 = self.decoder.to("cpu")
        self.decoder1 = self.decoder.to("cpu")
        self.lm_head = self.lm_head.to("cpu")
        self.model_parallel = False
        self.device_map = None
        torch.cuda.empty_cache()
    ...
    def set_input_embeddings(self, new_embeddings):
        self.shared = new_embeddings
        self.encoder.set_input_embeddings(new_embeddings)
        self.decoder0.set_input_embeddings(new_embeddings)
        self.decoder1.set_input_embeddings(new_embeddings)
    ...
    def get_decoder(self):
        if self.current_decoder == 0:
            return self.decoder0
        elif self.current_decoder == 1:
    ...
    def forward(
        ...
        if self.model_parallel:
            torch.cuda.set_device(self.decoder0.first_device)
            torch.cuda.set_device(self.decoder1.first_device)
        ...
        # Set device for model parallelism
        if self.model_parallel:
            torch.cuda.set_device(self.get_decoder().first_device)
            hidden_states = hidden_states.to(self.get_decoder().first_device)
            if decoder_input_ids is not None:
                decoder_input_ids = decoder_input_ids.to(self.get_decoder().first_device)
            if attention_mask is not None:
                attention_mask = attention_mask.to(self.get_decoder().first_device)
            if decoder_attention_mask is not None:
                decoder_attention_mask = decoder_attention_mask.to(self.get_decoder().first_device)
